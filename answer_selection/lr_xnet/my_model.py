...
class MY_Model:
  def __init__(self, sess, vocab_size):
    ...
    #self.cnt_placeholder = tf.placeholder(dtype, [None, FLAGS.max_doc_length], name='cnt-ph')
    self.isf_placeholder = tf.placeholder(dtype, [None, FLAGS.max_doc_length], name='isf-ph')
    self.idf_placeholder = tf.placeholder(dtype, [None, FLAGS.max_doc_length], name='idf-ph')
    #self.sent_len_placeholder = tf.placeholder(dtype, [None, FLAGS.max_doc_length], name='sent-len-ph')
    ...
    self.h_ensemble,\
    self.logits = model_ens_utils.policy_network_ens(self.vocab_embed_variable,
                                                      self.document_placeholder,
                                                      self.label_placeholder,
                                                      self.isf_placeholder,
                                                      self.idf_placeholder)
    
    self.probs = model_utils.convert_logits_to_sigmoid(self.logits_placeholder,None,False) # p(1) & p(0) are not mutually exclusive
    ### Define Supervised Cross Entropy Loss
    self.cross_entropy_loss = model_ens_utils.cross_entropy_loss(self.logits, self.label_placeholder, self.weight_placeholder)
    self.cross_entropy_loss_val = model_ens_utils.cross_entropy_loss(self.logits_placeholder,
