...
from data_utils import DataProcessor, BatchData, write_prediction_summaries, write_cos_sim, load_prediction
...
def batch_load_data(data):
  main_batch = BatchData(None,None,None,None,None,None,None,None)
  step = 1
  while (step * FLAGS.batch_size) <= len(data.filenames):
...
def batch_predict_with_a_model(batch,data_type,model, session=None, dump_emb=False):
  step = 1
  cos_sim_list = []
  logits_list = []
  sent_emb_list = []
  ...
  while (step * FLAGS.batch_size) <= len(batch.docs):
    ...
    if FLAGS.load_prediction==-1:
      if dump_emb:
        batch_logits,cos_sim,\
        sent_emb,question_emb = session.run([model.logits,
                                            model.cos_sim,
                                            model.sent_emb,
                                            model.question_emb],
        ...
        ques_emb_list.append(question_emb)
      else:
  ...
  if FLAGS.load_prediction!=-1:
    ...
    logits_list = load_prediction(fn_logits)
    cos_sim_list = load_prediction(fn_cos_sim)
    if dump_emb:
      fn = "step-a.model.ckpt.epoch-%d.%s-sent_emb" % (FLAGS.load_prediction,data_type)
      sent_emb_list = load_prediction(fn)
      fn = "step-a.model.ckpt.epoch-%d.%s-ques_emb" % (FLAGS.load_prediction,data_type)
      ques_emb_list = load_prediction(fn)
  else:
    # Concatenate logits and cos_sim
    logits_list = np.vstack(logits_list)
    cos_sim_list = np.vstack(cos_sim_list)
    if dump_emb:
      sent_emb_list = np.vstack(sent_emb_list)
      ques_emb_list = np.vstack(ques_emb_list)
  batch.logits = logits_list
  batch.cos_sim = cos_sim_list
  if dump_emb:
    return batch,sent_emb_list,ques_emb_list
  else:
    return batch



def dump_embeddings(batch,session):
  step = 1
  sent_emb_list = []
  ques_emb_list = []
  while (step * FLAGS.batch_size) <= len(batch.docs):
    ...
    sent_emb,ques_emb = session.run([model.sent_emb,model.question_emb],
                                      feed_dict={model.document_placeholder: docs,
                                                 model.label_placeholder: labels})
    sent_emb_list.append(sent_emb)
    ques_emb_list.append(ques_emb)
    # Increase step
    step += 1
  ...
  if (len(batch.docs) > ((step-1)*FLAGS.batch_size)):
    ...
    sent_emb_list.append(sent_emb)
    ques_emb_list.append(ques_emb)
  sent_emb = np.vstack(sent_emb_list)
  ques_emb = np.vstack(ques_emb)
  return sent_emb,ques_emb
  ...
  for epoch in range(init_epoch, FLAGS.train_epoch_crossentropy+1):
    ...
    ce_loss_val, ce_loss_sum, acc_sum = sess.run([ model.cross_entropy_loss_val,
                                                  ...
                                                  feed_dict={model.logits_placeholder: val_batch.logits,
                                                             ...
                                                             model.idf_score_placeholder:  val_batch.idf_score,
    ...
    #modelname = "step-a.model.ckpt.epoch-" + str(epoch)
...
def train_debug(sess):
  ...
  for epoch in range(init_epoch, FLAGS.train_epoch_crossentropy+1):
    ...
    while (step * FLAGS.batch_size) <= len(train_data.fileindices):
      ...
      # Print the progress
      if (step % FLAGS.training_checkpoint) == 0 or \
         ((step+1) * FLAGS.batch_size > len(train_data.fileindices)): # evaluate last 
        ...
        if ((step+1) * FLAGS.batch_size > len(train_data.fileindices)):
          print("Epoch %2d, step: %2d(%2d) -end-|| CE loss || Train : %4.3f , Val : %4.3f ||| ACC || Train : %.3f , Val : %.3f" % 
    ...
    if FLAGS.save_models:
      ...
      checkpoint_path = os.path.join(FLAGS.train_dir, "step-a.model.ckpt.epoch-"+str(epoch))
      model.saver.save(sess, checkpoint_path)
    print("Best Val ACC: %.6f | Epoch:%d" % (max_val_acc,best_ep))
  #END-FOR-EPOCH
...
def test_train(sess):
  ...
  FLAGS.authorise_gold_label = False
  test_batch = batch_predict_with_a_model(test_batch,"training",model, session=sess, dump_emb=FLAGS.save_preds)
